...
export default class Pages extends React.PureComponent {
  ...
  render() {
    const pages = this.pageRange().map((page) => {
      ...
      return <Page key={page} className={className} display={display} isCurrent={isCurrent} page={page} />
    });
    const firstPage = this.props.current !== 1 ?
      <Page key={'Start'} className='start' display='<span aria-hidden="true">&laquo;&laquo;</span>' page={1} />
      : null;
    const prevPage = this.props.current > 1 ?
      <Page key={'Previous'} className='previous' display='<span aria-hidden="true">&laquo;</span>' page={this.props.current - 1} />
      : null;
    const nextPage = this.props.current !== this.props.pages ?
      <Page key={'Next'} className='next' display='<span aria-hidden="true">&raquo;</span>' page={this.props.current + 1} />
      : null;
    const lastPage = this.props.current !== this.props.pages ?
      <Page key={'Last'} className='last' display='<span aria-hidden="true">&raquo;&raquo;</span>' page={this.props.current} />
      : null;
    return (
